...
class BackupStore {
  ...
  listFilenames(prefix, predicate, iteratees, parseWithoutKeys) {
    const options = {
      prefix: prefix
    };
    const self = this;
    ...
    return fetchFiles()
      .then(() =>
        _
          .chain(fileList)
          .map(file => {
            return !parseWithoutKeys ? this.filename.parse(file.name) : this.filename.parseWithoutKeys(file.name);
          })
          .filter(predicate)
          .sortBy(iteratees)
          .value());
  ...
  listBackupFilenames(backupStartedBefore, options, parseWithoutKeys) {
    const iteratees = ['started_at'];
    ...
    return Promise
      ...
      .then(isoDate => this.listFilenames(this.getFileNamePrefix(options), getPredicate(isoDate), iteratees, parseWithoutKeys));
  }
...
class Filename {
  ...
  /**
   * 
   * Assumption is that the filename is of the pattern <root folder>/<operation>/<dot separated values>.<backup_guid>.yyyy-mm-ddTHH-mm-ssZ.json
   */
  parseWithoutKeys(filename) {
    ...
    let filePathValues = path.basename(filename).split('.');

    if (filePathValues !== undefined && filePathValues.length > 3) {
      data.baseFileName = path.basename(filename);
      data.started_at = filePathValues[filePathValues.length - 2];
      data.backup_guid = filePathValues[filePathValues.length - 3];
    }
